...
def counts_report(subset_array):
    counts = {0: 0, 1: 0, 2: 0, 3: 0}
    for word in subset_array:
        for val in word:
            counts[int(val)] += 1
    return counts
...
if __name__ == "__main__":
    ...
    counts = counts_report(subset)
    print(
        f"✅ subset length: {len(subset)} "
        f"(0s={counts[0]}, 1s={counts[1]}, 2s={counts[2]}, 3s={counts[3]})"
    )
